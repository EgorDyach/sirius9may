...
export function GallaryPagination({activeIndex, setActiveIndex}: {activeIndex: number; setActiveIndex: React.Dispatch<React.SetStateAction<number>>}) {
  ...
  for (let i = 0; i < swiper.slides.length; i++) {
    slides.push( <button className={activeIndex === i ? 'gallary__pagination-btn gallary__pagination-btn-active' : 'gallary__pagination-btn'} onClick={() => { swiper.slideTo(i); setActiveIndex(i) }}>
