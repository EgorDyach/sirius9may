...
export function PersonNav({ activeIndex, count, setActiveIndex }: { activeIndex: number; count: number; setActiveIndex: React.Dispatch<React.SetStateAction<number>> }) {
  ...
  return (
    <div className="gallary__controls">
      <div className='gallary__nav'>
        ...
        <button disabled={activeIndex >= count - 1} onClick={() => { swiper.slideNext(); setActiveIndex(activeIndex + (windowWidth > 1000 ? 4 : 1)) }} className='gal-swiper-button-next'>
          <svg width="50" height="50" viewBox="0 0 50 50" fill="none" xmlns="http://www.w3.org/2000/svg">
