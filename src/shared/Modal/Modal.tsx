...
import ModalSkipToActive from './ModalSkipToActive';
import { ModalNav } from '../ModalNav';
import other from '../../assets/medals/medalOther.png'
export function Modal() {
  const isOpen = useAppSelector(state => state.modalSlice.isOpen);
  const img = useAppSelector(state => state.modalSlice.images);
  const active = useAppSelector(state => state.modalSlice.activeIndex);
  useLayoutEffect(() => {
        ...
        <div className="modal__content">
          <button onClick={() => {
            ...
            <svg width="90" height="90" viewBox="0 0 90 90" fill="none" xmlns="http://www.w3.org/2000/svg">
              <rect x="-2.5" y="2.5" width="85" height="85" transform="matrix(-1 0 0 1 85 0)" stroke="#343434" strokeWidth="5" />
          ...
          <Swiper
            ...
            {img.map(e => {
              return <SwiperSlide>
                <img src={e.type === 'personPhoto' ? e.src : e.type === 'gallaryPhoto' ? e.src : (!e.text ? other :`/medals/${getMedalKeyByName(e.text)}.png`)} />
                <Text weight={400} size={20} color='#fff'>{e.text || (e.type === 'gallaryPhoto' ? e.date : 'Неизвестная медаль')}</Text>
                {<ModalSkipToActive active={active} />}
              </SwiperSlide>
            })}
            {windowWidth > 1400 && <SwiperSlide></SwiperSlide>}
